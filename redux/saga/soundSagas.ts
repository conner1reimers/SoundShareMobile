...
import { FETCH_SINGLE_SOUND, FETCH_SINGLE_SOUND_ASYNC } from '../store/actions/actionTypes';
...
// function* fetchRecentServerAsync(action: any) {
//   try {
...
function* recentSoundsFetchMoreAsync(action: any) {
  try {
    yield call(loadAndCall,
...
function* fetchRecentCategoryAsync(action: any) {
...
function* fetchTopDownloadsAsync(action: any) {
  try {
...
function* refreshAllDownloadsAsync(action: any) {
  try {
    yield call(loadAndCall,
...
function* refreshAllLikesAsync(action: any) {
...
function* fetchTopLikedAsync(action: any) {
  try {
...
function* fetchSingleSoundAsync(action: any) {
  try {
    yield call(loadAndCall, async () => await sendRequest(`/sounds/${action.sid}`), startAction(action.type),
      stopAction(action.type), FETCH_SINGLE_SOUND_ASYNC, action.id, false, true);
  } catch(err) {}
}
