...
const GlobalPlayer: FC = (props: Props) => {
    ...
    return {
      transform: playerOpen.value
        ? [
            ...
            {
              scale: isPinching.value
                ? pinchScale.value
                : withSpring(
                    isPressed.value && pinchScale.value === 1
                      ? 1.05
                      : isPressed.value && pinchScale.value !== 1
                      ? pinchScale.value
                      : !isPressed.value && pinchScale.value !== 1
                      ...
                      : 1,
                    {
                      ...
                      mass: playerOpen && !isPressed.value ? 0.5 : isPressed.value ? 1 : 0.2,
    ...
    .onUpdate((e) => {
      if (isPressed.value) {
        ...
        const updatedDistance = Math.hypot(
          offset.value.x - updatedBackdropX,
          offset.value.y - updatedBackdropY
        );
        ...
        if (newScale >= 1) {
          ...
          pinchScale.value = Math.max(newScale, 0.7);
        }
      }
    })
    .onEnd(() => {})
  ...
  const composed = Gesture.Simultaneous(gesture, Gesture.Simultaneous(zoomGesture, rotateGesture));
  ...
  return (
    <>
      ...
      <GestureDetector gesture={backdropGesture}>
        <Animated.View style={[styles.backdrop, backdropGesturestyles]}></Animated.View>
      </GestureDetector>
    </>
  );
};
